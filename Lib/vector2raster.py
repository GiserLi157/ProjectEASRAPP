...
from osgeo.gdal import GetDriverByName, GA_ReadOnly, Open as gdalOpen, SetConfigOption, RasterizeLayer, GDT_Byte, GDT_Float32
from os import path
...
class vector2raster:
    ...
    def set_resolution(self):
        ...
        if self.referRaster:
            ...
            self.e3_text.set(str(resolution))
    ...
    def getpath(self):
        shpPath, rasterPath, resolution, field  = self.e1_text.get(), self.e2_text.get(), abs(float(self.e3_text.get())), self.combobox.get()
        self.root.destroy()
    ...
    def vector_to_raster(self, shapefiles, savePath, res_size, field):
        ...
        for shapefile in shapefiles:
            ...
            width = int((x_max - x_min) / res_size + 0.5)
            height = int((y_max - y_min) / res_size + 0.5)
            if field == "None":
            ...
            geotrans = (x_min, res_size, 0, y_max, 0, -res_size)
